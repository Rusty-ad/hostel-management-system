			...
			<div class="row">
				...
				<div class="col-lg-6 col-md-6 col-xs-12">
					<div class="page-header">
						<center><h1>Remove Student from the Hostel</h1></center>      
					</div>
					<?php
						error_reporting(0);
						$conn = new mysqli($host,$user,$pass,$db);
						$q="SELECT id, Reg_name, name, father, rooms FROM user"; 
						$result=$conn->query($q);
						$i=1;
						if($result->num_rows>0){
							...
							while($row=$result->fetch_assoc()){
									...
									$row["Reg_name"] . "</td>" . "<td>" . $row["father"] ."</td>". "<td>" . $row["rooms"] ."</td>". "<td>";
									$i++;
							}
							echo "</table></div>";
						}else{
					...
					<form action="remove-confirmation.php" method="POST">
						...
						<input type="text" name="reg_id" required /><br><br>
						<label for="room">Room Number:</label>
						<input type="text" name="room" required /><br><br>
						<input type="submit" name="submit" value="Remove" id="button" onclick="return confirm('Are you sure to remove this student from the hostel?');">
